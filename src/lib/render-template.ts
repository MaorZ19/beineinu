...
export interface QuoteForRender {
  ...
  recipient: string;
  sentAt: string | Date;
  category?: string; // affection | family | funny | meaningful
}

/**
 * The name shown on the card. Maor's idea: when a quote is ABOUT their child
 * (category 'family'), sign it as the parent role — "אמא" if the wife
 * (מאורי) said it, "אבא" if Maor (מאור) said it. Otherwise use the name.
 */
export function signatureName(speaker: string, category?: string): string {
  if (category === "family") {
    return speaker === "מאורי" ? "אמא" : "אבא";
  }
  return speaker;
}
...
export function renderQuoteHtml(q: QuoteForRender): string {
  ...
  const speaker = escapeHtml(signatureName(q.speaker, q.category));
